...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            checkboxInput("rollList", "Show List of Dice Rolls", value = FALSE), # checkbox to show list of dice rolls
            checkboxInput("plot", "Plot Dice Rolls"), # checkbox to plot list of dice rolls
            actionButton("roll", "Roll Dice") # button to roll dice
        ),
        mainPanel(
            textOutput("diceRollSum"), # dice roll sum text output
...
server <- function(input, output) {
    ...
    observeEvent(input$roll, {
        ...
        # sort dice rolls
        sortedDiceRolls <- sortedDiceRollsTable[
            order(sortedDiceRollsTable$Freq, decreasing = TRUE),
        ]
        # set output to sum of dice rolls
        output$diceRollSum <- renderText(paste("Total Dice: ", sum(diceRolls)))
        # set output to values of dice rolls
        output$diceRollValues <- renderText(paste(
            "Dice Rolls by Number: \n",
            paste(
                names(diceRollsTable),
                diceRollsTable,
                sep = ": ",
                collapse = "\n"
            )
        ...
        output$sortedDiceRollValues <- renderText(paste(
            "Sorted Dice Rolls by Number: \n",
            paste(
                sortedDiceRolls$diceRolls,
                sortedDiceRolls$Freq,
                sep = ": ",
                collapse = "\n"
            )
        ))
        ...
        if (input$rollList) {
            output$diceRollList <- renderText(paste(
                "List of Dice Rolls: \n",
